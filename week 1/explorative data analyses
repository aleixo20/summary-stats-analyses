...
sns.pairplot(df, x_vars=['peak_memory_mb', 'average_memory_used_as_mb', 'percent_cpu', 'cpus_requested'], y_vars='memory_requested_mb', height=5, aspect=0.7)
plt.show()
...
def linear_regression_analysis(feature):
    ...
    plt.xlabel(feature)
    plt.ylabel('Memory Requested (MB)')
    plt.title(f'Regression: {feature} vs Memory Requested')
    plt.show()
...
for feature in ['peak_memory_mb', 'average_memory_used_as_mb', 'percent_cpu', 'cpus_requested']:
    linear_regression_analysis(feature)

# Multiple linear regression
X = df[['peak_memory_mb', 'average_memory_used_as_mb', 'percent_cpu', 'cpus_requested']]
y = df['memory_requested_mb']
